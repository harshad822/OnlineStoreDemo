...
public class UserRepository : IUserRepository
{
    ...
    public UserRepository(OnlineStoreDbContext db, IConfiguration configuration)
    {
        _db = db;
        ...
        try
        {
            if (model.Email is not null && model.Username is not null)
            {
                ...
                if (userExists is not null)
                    return new ResponseModel(false, "Username or Email address has already been registered.", StatusCodes.Status302Found);
                ...
                Data.Entity.DbSet.User user = new()
                {
                    ...
                    Password = PasswordHasher.Encrypt(model.Password),
                    IsAdmin = model.IsAdmin
                };
                await _db.Users.AddAsync(user);
                await _db.SaveChangesAsync();
                return new ResponseModel(true, "User Created Successfully", StatusCodes.Status201Created);
            }
            else
            {
                return new ResponseModel(false, "Invalid or partial data entered", StatusCodes.Status400BadRequest);
            }
        }
        catch (Exception ex)
        {
            return new ResponseModel(false, "Something went wrong! Please try again after sometime.", StatusCodes.Status500InternalServerError);
        }
    }

    public async Task<ResponseResult<UserModel>> Login(LoginModel model)
    {
        ...
        if (model != null)
        {
            try
            {
                ...
                if (user != null)
                {
                    var validpassword = await _db.Users.Where(x => x.Username == model.Username && x.Password == PasswordHasher.Encrypt(model.Password)).FirstOrDefaultAsync();
