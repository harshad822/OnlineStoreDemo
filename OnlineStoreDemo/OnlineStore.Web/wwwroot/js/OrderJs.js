$(document).ready(function () {
        ...
        columns: [
            ...
            { data: 'status' },
            {
                data: null, sorting:false,className:"td-actions", render: function (data, type, row) {
                    var button = '<button class="btn btn-dark btn-just-icon btn-sm view-order"><i class="material-symbols-outlined">visibility</i></button>';
                    return button;
                }
            }
        ]
    ...
    window.updateViewOrderButtons = function () {
        $('.view-order').off('click').on('click', function () {
            ...
            $("#orderDate").text(DateFormatter_DDMMYYYY(data.createdAt));
            var order = {
                Id: data.id
            }
    ...
    window.DateFormatter_DDMMYYYY = function (value) {
        if (value) {
